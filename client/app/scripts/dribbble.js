...
angular.module('clientApp')
  ...
  .controller('DribbbleCtrl', ['$scope', '$rootScope', '$http', '$window', 'ModalService',
    function($scope, $rootScope, $http, $window, ModalService) {
      $scope.user = {};
      $scope.posts = [];

      var streamElement = angular.element(document.getElementById('dribbble-page'))[0];
      ...
      function _getPosts(cb) {
        ...
        fetching = true;
        $http.get('/dribbble/' + currentPage).success(function(data, status) {
      ...
      function _getUser(cb) {
        $http.get('/dribbble/user').success(function(data, status) {
          if (status === 200 && data) {
            $scope.user = data;
            _getPosts(cb);
          }
      ...
      $scope.openPost = function(item) {
        ModalService.showModal({
          templateUrl: 'templates/dribbble/details.html',
          controller: 'DribbbleDetailsCtrl',
          inputs: {
            item: item
          }
        });
  ...
  .controller('DribbbleDetailsCtrl', ['$scope', 'close', 'item',
